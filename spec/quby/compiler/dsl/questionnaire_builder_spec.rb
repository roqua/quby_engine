  ...
  describe QuestionnaireBuilder do
    ...
    it 'raises when a depends_on key does not exist in the questionnaire' do
      expect do
        ...
        questionnaire.callback_after_dsl_enhance_on_questions
      end.to raise_exception(Quby::Compiler::Entities::Questionnaire::UnknownInputKey, "Question v_1 depends_on contains an error: Unknown input key unknown")
    end
    ...
    describe 'flag' do
      before do
        allow_any_instance_of(Quby::Compiler::Entities::Questionnaire).to receive(:add_flag)
      end
