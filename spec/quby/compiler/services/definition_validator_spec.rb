...
module Quby::Compiler::Services
  describe DefinitionValidator do
    let(:questionnaire) { Quby::Compiler::DSL.build('test', 'title "Test"') }

    def make_definition(definition = 'title "Test"')
      Quby::Compiler::Entities::Definition.new(key: 'test', sourcecode: definition, path: 'fake definition test')
    end

    describe "fields#answer_keys and fields#input_keys need to be symbols" do
      before do
        allow(Quby::Compiler::DSL).to receive(:build_from_definition).and_return(questionnaire)
      end
