...
module ServerSideValidationHelpers
  ...
  def expect_error_on(question_key, error_type)
    expect(answer.errors[question_key].map { |error| error[:valtype].to_s }).to include(error_type.to_s)
  end

  def expect_no_error_on(question_key)
    expect(answer.errors[question_key].map { |error| error[:valtype] }).to be_empty
  end
end
