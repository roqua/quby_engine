class Questionnaire < ActiveRecord::Base
  ...
  def write_to_disk
    ...
    unless Rails.env.development?
      system "cd #{Rails.root}/db/questionnaires && git config --global user.name \"quby #{Rails.root.parent.parent.basename.to_str}, user: #{@last_author}\" && git add . && git commit -m 'auto-commit from admin' &&  git push"
    end
  end
