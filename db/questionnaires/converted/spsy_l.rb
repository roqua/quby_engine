# SPsy Leraren

# Project ID 205107
# Date (GMT) 15-01-2010 09:01:43

key "spsy_l"
title "SPsy Leerkrachten"
description ""

question :q1866970, :type => :open do
  title "Vul de gegevens van uw school in:"
  description ""
end

question :q104, :type => :open do
  title "Adres"
  description ""
end

question :q860405, :type => :radio do
  title "1. Probeert aardig te zijn tegen anderen. Houdt rekening met hun gevoelens."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860411, :type => :radio do
  title "2. Is rusteloos, overactief, kan niet lang stilzitten."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860414, :type => :radio do
  title "3. Klaagt vaak over hoofdpijn, buikpijn, of misselijkheid."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860418, :type => :radio do
  title "4. Deelt makkelijk met andere kinderen (snoep, speelgoed, potloden, enz)."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860420, :type => :radio do
  title "5. Wordt erg boos en is vaak driftig."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860422, :type => :radio do
  title "6. Is nogal op zichzelf. Speelt meestal alleen of bemoeit zich niet met anderen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860424, :type => :radio do
  title "7. Doet meestal wat hem/haar wordt opgedragen."
  description ""
  option :a1, :value => 1, :description => "Zeker waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Niet waar"
end

question :q860427, :type => :radio do
  title "8. Piekert veel."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860431, :type => :radio do
  title "9. Helpt iemand die zich heeft bezeerd, van streek is of zich ziek voelt."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860433, :type => :radio do
  title "10. Zit constant te wiebelen of te friemelen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860435, :type => :radio do
  title "11. Heeft minstens één goede vriend of vriendin."
  description ""
  option :a1, :value => 1, :description => "Zeker waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Niet waar"
end

question :q860437, :type => :radio do
  title "12. Vecht vaak. Het lukt uw kind andere mensen te laten doen wat hij/zij wil."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860440, :type => :radio do
  title "13. Is vaak ongelukkig, in de put of in tranen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860442, :type => :radio do
  title "14. Wordt over het algemeen aardig gevonden door andere kinderen."
  description ""
  option :a1, :value => 1, :description => "Zeker waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Niet waar"
end

question :q860444, :type => :radio do
  title "15. Is snel afgeleid, vindt het moeilijk zich te concentreren."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860447, :type => :radio do
  title "16. Is zenuwachtig in nieuwe situaties. Verliest makkelijk zelfvertrouwen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860449, :type => :radio do
  title "17. Is aardig tegen jonge kinderen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860451, :type => :radio do
  title "18. Liegt of bedriegt vaak."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860454, :type => :radio do
  title "19. Wordt getreiterd of gepest door andere kinderen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860456, :type => :radio do
  title "20. Biedt vaak vrijwillig hulp aan anderen (ouders, leerkrachten, andere kinderen)."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860458, :type => :radio do
  title "21. Denkt na voor iets te doen."
  description ""
  option :a1, :value => 1, :description => "Zeker waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Niet waar"
end

question :q860461, :type => :radio do
  title "22. Pikt dingen thuis, op school of op andere plaatsen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860463, :type => :radio do
  title "23. Kan beter opschieten met volwassenen dan met leeftijdgenoten."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860465, :type => :radio do
  title "24. Is voor heel veel dingen bang, is snel angstig."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860468, :type => :radio do
  title "25. Maakt af waar hij/zij mee bezig is. Kan de aandacht er goed bijhouden."
  description ""
  option :a1, :value => 1, :description => "Zeker waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Niet waar"
end

question :q860471, :type => :radio do
  title "26. Heeft het gevoel gehad alsof andere mensen zijn/haar gedachten kunnen lezen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860475, :type => :radio do
  title "27. Kan zich vaak niet beheersen en eet dan enorm veel."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860479, :type => :radio do
  title "28. Heeft expres geprobeerd zichzelf iets aan te doen (bijvoorbeeld snijden, slaan)."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860481, :type => :radio do
  title "29. Doet erg zijn/haar best om af te vallen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860483, :type => :radio do
  title "30. Heeft het gevoel gehad boodschappen te krijgen via de radio of televisie die alleen voor hem/haar bestemd zijn."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860485, :type => :radio do
  title "31. Vindt zichzelf dik, ook al zeggen anderen dat dit niet zo is."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860488, :type => :radio do
  title "32. Heeft in de afgelopen week erover nagedacht een einde aan het leven te maken."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860490, :type => :radio do
  title "33. Heeft het gevoel gehad achtervolgd en bespioneerd te worden."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860493, :type => :radio do
  title "34. Geeft soms expres over na het eten."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860498, :type => :radio do
  title "35. Heeft het gevoel gehad stemmen te horen die andere mensen niet kunnen horen."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860508, :type => :radio do
  title "36. Vindt dat eten zijn/haar leven beheerst."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860565, :type => :radio do
  title "37. Gebruikt alcohol:"
  description ""
  option :a1, :value => 1, :description => "Niet in de afgelopen zes maanden"
  option :a2, :value => 2, :description => "Eens per maand of minder"
  option :a3, :value => 3, :description => "2 tot 4 keer per maand"
  option :a4, :value => 4, :description => "2 tot 3 keer per week"
  option :a5, :value => 5, :description => "4 keer per week of vaker"
end

question :q860575, :type => :radio do
  title "38. Zou wel eens wat minder alcohol mogen gebruiken."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860577, :type => :radio do
  title "39. Heeft problemen gehad door zijn/haar alcoholgebruik (op school, thuis of werk)."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860581, :type => :radio do
  title "40. Iemand (een familielid of vriend) heeft bezorgdheid geuit over het alcoholgebruik van dit kind of hem/haar gezegd te stoppen met het gebruik van alcohol."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860592, :type => :radio do
  title "41. Gebruikt drugs:"
  description ""
  option :a1, :value => 1, :description => "Niet in de afgelopen zes maanden"
  option :a2, :value => 2, :description => "Eens per maand of minder"
  option :a3, :value => 3, :description => "2 tot 4 keer per maand"
  option :a4, :value => 4, :description => "2 tot 3 keer per week"
  option :a5, :value => 5, :description => "4 keer per week of vaker"
end

question :q860595, :type => :radio do
  title "42. Zou wel eens wat minder drugs mogen gebruiken."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860597, :type => :radio do
  title "43. Heeft problemen gehad door zijn/haar drugsgebruik (op school, thuis of werk)."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860599, :type => :radio do
  title "44. Iemand (een familielid of vriend) heeft bezorgdheid geuit over het drugsgebruik van dit kind of hem/haar gezegd te stoppen met het gebruik van drugs."
  description ""
  option :a1, :value => 1, :description => "Niet waar"
  option :a2, :value => 2, :description => "Beetje waar"
  option :a3, :value => 3, :description => "Zeker waar"
end

question :q860609, :type => :radio do
  title "45. Denkt u over het geheel genomen dat dit kind moeilijkheden heeft op één van de volgende gebieden: emoties, concentratie, gedrag of vermogen om met andere mensen op te schieten?"
  description ""
  option :a1, :value => 1, :description => "Nee"
  option :a2, :value => 2, :description => "Ja, kleine moeilijkheden"
  option :a3, :value => 3, :description => "Ja, duidelijke moeilijkheden"
  option :a4, :value => 4, :description => "Ja, ernstige moeilijkheden"
end

question :q860613, :type => :radio do
  title "46. Hoe lang bestaan deze moeilijkheden?"
  description ""
  option :a1, :value => 1, :description => "Korter dan een maand"
  option :a2, :value => 2, :description => "1 tot 5 maanden"
  option :a3, :value => 3, :description => "6 tot 12 maanden"
  option :a4, :value => 4, :description => "Meer dan een jaar"
end

question :q860614, :type => :radio do
  title "47. Maken de moeilijkheden dit kind overstuur of van slag?"
  description ""
  option :a1, :value => 1, :description => "Helemaal niet"
  option :a2, :value => 2, :description => "Een beetje maar"
  option :a3, :value => 3, :description => "Tamelijk"
  option :a4, :value => 4, :description => "Heel erg"
end

question :q860615, :type => :radio do
  title "48. Belemmeren de moeilijkheden het dagelijkse leven van dit kind op de volgende gebieden?"
  description ""
end

question :q98, :type => :radio do
  title "Leren in de klas"
  description ""
  option :a1, :value => 1, :description => "Helemaal niet"
  option :a2, :value => 2, :description => "Een beetje maar"
  option :a3, :value => 3, :description => "Tamelijk"
  option :a4, :value => 4, :description => "Heel erg"
end

question :q102, :type => :radio do
  title "Contacten met leeftijdsgenoten"
  description ""
  option :a1, :value => 1, :description => "Helemaal niet"
  option :a2, :value => 2, :description => "Een beetje maar"
  option :a3, :value => 3, :description => "Tamelijk"
  option :a4, :value => 4, :description => "Heel erg"
end

question :q860619, :type => :radio do
  title "49. Belasten de moeilijkheden u of de klas als geheel?"
  description ""
  option :a1, :value => 1, :description => "Helemaal niet"
  option :a2, :value => 2, :description => "Een beetje maar"
  option :a3, :value => 3, :description => "Tamelijk"
  option :a4, :value => 4, :description => "Heel erg"
end

question :q860621, :type => :open do
  title "50. Overige opmerkingen:"
  description ""
end

